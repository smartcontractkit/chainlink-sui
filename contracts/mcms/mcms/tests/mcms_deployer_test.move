...
fun proof_package_address(): address {
    let proof_type = type_name::with_original_ids<MCMS_DEPLOYER_TEST>();
    let proof_address_bytes = proof_type.address_string().into_bytes();
    address::from_ascii_bytes(&proof_address_bytes)
}
...
fun setup_registered_package_with_upgrade_cap(scenario: &mut Scenario) {
    ...
    mcms_deployer::test_register_upgrade_cap_with_original(
        &mut deployer_state,
        upgrade_cap,
        proof_package_address(),
        ctx,
    );
...
// The proof's registered binding resolves to the cap ID at the current address.
#[test]
fun test_release_upgrade_cap_before_upgrade() {
...
// The proof-to-cap binding remains stable across repeated re-keys, and release succeeds only from
// the address containing that exact cap.
#[test]
fun test_release_upgrade_cap_after_upgrade() {
    let mut scenario = create_test_scenario();
    setup_registered_package_with_upgrade_cap(&mut scenario);

    let new_package_address: address;

    // Perform two upgrades so the test covers a stable cap binding across repeated re-keys.
    {
        ts::next_tx(&mut scenario, @0xA);
        let mut deployer_state = ts::take_shared<DeployerState>(&scenario);
        ...
        let first_package_address = receipt.package().to_address();
        mcms_deployer::commit_upgrade(&mut deployer_state, receipt, ctx);

        assert!(!mcms_deployer::has_upgrade_cap(&deployer_state, TEST_PACKAGE), 0);
        assert!(mcms_deployer::has_upgrade_cap(&deployer_state, first_package_address), 1);
        ...
        assert!(!mcms_deployer::has_upgrade_cap(&deployer_state, first_package_address), 2);
        assert!(mcms_deployer::has_upgrade_cap(&deployer_state, new_package_address), 3);
...
#[test]
fun test_bind_legacy_upgrade_cap() {
    let mut scenario = create_test_scenario();
    ...
    let upgrade_cap = package::test_publish(TEST_PACKAGE.to_id(), ctx);
    mcms_deployer::test_register_upgrade_cap_without_binding(
        &mut deployer_state,
        upgrade_cap,
        ctx,
    );

    let mut data = bcs::to_bytes(&object::id_address(&registry));
    data.append(bcs::to_bytes(&object::id_address(&deployer_state)));
    data.append(bcs::to_bytes(&proof_package_address()));
    data.append(bcs::to_bytes(&TEST_PACKAGE));
    ...
    let params = mcms_registry::test_create_executing_callback_params(
        ...
        string::utf8(b"bind_legacy_upgrade_cap"),
        data,
        vector[1],
        0,
        1,
    );
    mcms_deployer::bind_legacy_upgrade_cap(
        &mut deployer_state,
        &mut registry,
        params,
    );
