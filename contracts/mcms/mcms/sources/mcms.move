...
public entry fun set_root(
    ...
    assert!(clock.timestamp_ms() <= valid_until, EValidUntilExpired);
...
public fun execute_timelock_schedule_batch(
    ...
    let (_, _, _, data) = get_callback_params_for_mcms(
        executing_callback_params,
        b"timelock_schedule_batch",
    );
...
public fun execute_timelock_bypasser_execute_batch(
    ...
    let (_, _, _, data) = get_callback_params_for_mcms(
        executing_callback_params,
        b"timelock_bypasser_execute_batch",
    );
...
fun get_callback_params_for_mcms(
    executing_callback_params: ExecutingCallbackParams,
    expected_function_name: vector<u8>,
): (address, String, String, vector<u8>) {
    let (target, module_name, function_name, data) = mcms_registry::get_callback_params_for_mcms(
        executing_callback_params,
    );
    assert!(target == @mcms, EWrongMultisig);
    assert!(*module_name.as_bytes() == b"mcms", EInvalidModuleName);
