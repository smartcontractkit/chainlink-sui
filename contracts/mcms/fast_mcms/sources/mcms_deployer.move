...
fun resolve_upgrade_cap_address(
    state: &DeployerState,
    original_package_address: address,
): address {
    ...
    let upgrade_cap_id = *df::borrow<UpgradeCapBindingKey, ID>(
        &state.id,
...
fun add_upgrade_cap_binding(
    ...
    assert!(
        !has_upgrade_cap_binding(state, original_package_address),
        EUpgradeCapAlreadyBound,
    );
...
/// The `ExecutingCallbackParams` can only be created by MCMS after timelock authorization.
public fun bind_legacy_upgrade_cap(
    state: &mut DeployerState,
    registry: &mut Registry,
    params: ExecutingCallbackParams,
) {
    let (target, module_name, function_name, data) =
        mcms_registry::get_callback_params_from_mcms(registry, params);
    ...
    assert!(
        *function_name.as_bytes() == b"bind_legacy_upgrade_cap",
        EInvalidMCMSCallbackFunction,
    );
    ...
    assert!(
        state.upgrade_caps.contains(current_package_address),
        EPackageAddressNotRegistered,
    );
    ...
    let upgrade_cap_id = object::id(upgrade_cap);
    assert!(
        upgrade_cap.package().to_address() == current_package_address,
        EPackageAddressMismatch,
    );
    assert!(
