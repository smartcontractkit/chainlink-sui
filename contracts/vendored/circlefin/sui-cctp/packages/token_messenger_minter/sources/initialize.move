...
/// Module: initialize
/// This module contains initialization code for the  
/// TokenMessengerMinter package.
module token_messenger_minter::initialize {
  // === Imports ===
  use token_messenger_minter::state::{Self};
  ...
  /// Initializes state with given message_body_version, all roles set to the tx sender,
  /// and paused set to false.
  ...
  public fun init_state(init_cap: InitCap, message_body_version: u32, ctx: &mut TxContext) {
  ...
  #[test_only] use token_messenger_minter::state::{State};
  ...
  fun test_init_state_successful() {
    ...
    {
      init_state(init_cap, 0, scenario.ctx());
