...
module ccip_offramp::offramp_test {
    ...
    public fun test_initialize() {
        ...
        let (chain_selector, rmn_remote, token_admin_registry, nonce_manager) = 
        ...
        let (router, is_enabled, min_seq_nr, is_rmn_disabled, on_ramp) = 
    ...
    public fun test_apply_source_chain_config_updates() {
        ...
        let (_, is_enabled, _, is_rmn_disabled, _) = 
            offramp::get_source_chain_config_fields(updated_config);
        assert!(is_enabled == true);
        assert!(is_rmn_disabled == false);
    ...
    public fun test_set_ocr3_config() {
        ...
        let (digest, f, n, sig_verification, config_signers, config_transmitters) = 
        ...
        let (_, _, _, exec_sig_verification, _, _) = 
            offramp::latest_config_digest_fields(exec_config);
    ...
    public fun test_config_signers_and_transmitters() {
        ...
        assert!(config_signers.length() == 5);
        assert!(config_signers[0] == x"1111111111111111111111111111111111111111111111111111111111111111");
        assert!(config_signers[4] == x"5555555555555555555555555555555555555555555555555555555555555555");
