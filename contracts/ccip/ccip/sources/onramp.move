...
use ccip::onramp_state_helper::{Self as osh, SourceTransferCap, TokenTransferParams};
...
public struct OnRampState has key, store {
    ...
    source_transfer_cap: Option<SourceTransferCap>,
...
const EInvalidFunctionVersion: u64 = 20;
...
fun init(_witness: ONRAMP, ctx: &mut TxContext) {
    ...
    let tn = type_name::with_defining_ids<ONRAMP>();
...
public fun is_cursed_function(ref: &OnRampState, function: vector<u8>): bool {
    ref.cursed_functions.contains(&function)
}

public fun curse_function(ref: &mut OnRampState, _: &OwnerCap, function: vector<u8>) {
    ref.cursed_functions.insert(function);
}

public fun uncurse_function(ref: &mut OnRampState, _: &OwnerCap, function: vector<u8>) {
...
public fun withdraw_fee_tokens<T>(
    ...
    assert!(
        !is_cursed_function(state, b"onramp::withdraw_fee_tokens::v1"),
...
public fun get_fee<T>(
    ...
    assert!(
        !state_object::is_cursed_function(ref, b"onramp::get_fee::v1"),
        EInvalidFunctionVersion,
    );
    get_fee_internal(
        ref,
...
public fun set_dynamic_config(
    ...
    assert!(!is_cursed_function(state, b"onramp::set_dynamic_config::v1"), EInvalidFunctionVersion);
    set_dynamic_config_internal(state, fee_aggregator, allowlist_admin);
}
...
public fun apply_dest_chain_config_updates(
    ...
    assert!(
        !is_cursed_function(state, b"onramp::apply_dest_chain_config_updates::v1"),
        EInvalidFunctionVersion,
    );
    apply_dest_chain_config_updates_internal(
        state,
...
public fun apply_allowlist_updates(
    ...
    assert!(
        !is_cursed_function(state, b"onramp::apply_allowlist_updates::v1"),
        EInvalidFunctionVersion,
    );
...
public fun apply_allowlist_updates_by_admin(
    ...
    assert!(
        !is_cursed_function(state, b"onramp::apply_allowlist_updates_by_admin::v1"),
        EInvalidFunctionVersion,
    );
...
public fun ccip_send<T>(
    ...
    assert!(!is_cursed_function(state, b"onramp::ccip_send::v1"), EInvalidFunctionVersion);
