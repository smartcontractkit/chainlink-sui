...
public struct UpgradeRegistry has key, store {
    id: UID,
    // Maps (module_name, function_name) -> blocked functionversions
...
public fun is_function_allowed(
    ...
    if (!is_module_allowed(registry, module_name, contract_version)) {
        return false
    };
...
public fun get_module_restrictions(registry: &UpgradeRegistry, module_name: String): vector<u64> {
    if (!registry.module_restrictions.contains(module_name)) {
        vector::empty()
    } else {
