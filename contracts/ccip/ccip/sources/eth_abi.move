...
public fun encode_left_padded_bytes32(out: &mut vector<u8>, value: vector<u8>) {
...
public fun encode_right_padded_bytes32(out: &mut vector<u8>, value: vector<u8>) {
...
public fun encode_packed_bytes(out: &mut vector<u8>, value: vector<u8>) {
    out.append(value);
}

public fun encode_packed_bytes32(out: &mut vector<u8>, value: vector<u8>) {
    assert!(value.length() <= 32, EInvalidBytes32Length);
    out.append(value);
...
public fun decode_address(stream: &mut ABIStream): address {
    ...
    while (i < 12) {
        assert!(data[cur + i] == 0, EInvalidAddress);
...
public fun decode_bool(stream: &mut ABIStream): bool {
    ...
    if (value == ENCODED_BOOL_FALSE) { false } else if (value == ENCODED_BOOL_TRUE) { true } else {
        abort EInvalidBool
    }
}
...
public fun decode_bytes(stream: &mut ABIStream): vector<u8> {
    ...
    let padding_len = if (length % 32 == 0) { 0 } else {
        32 - (length % 32)
    };
    ...
    assert!(cur + length + padding_len <= data.length(), EOutOfBytes);
...
public fun decode_u256_value(mut value_bytes: vector<u8>): u256 {
    assert!(value_bytes.length() == 32, EInvalidU256Length);
    value_bytes.reverse();
