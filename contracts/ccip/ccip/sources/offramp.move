...
use ccip::ocr3_base::{Self, OCR3BaseState, OCRConfig};
...
public struct OffRampState has key, store {
    ...
    dest_transfer_cap: Option<DestTransferCap>,
...
const EInvalidFunctionVersion: u64 = 30;
...
public fun is_cursed_function(ref: &OffRampState, function: vector<u8>): bool {
    ref.cursed_functions.contains(&function)
}

public fun curse_function(ref: &mut OffRampState, _: &OwnerCap, function: vector<u8>) {
    ref.cursed_functions.insert(function);
}

public fun uncurse_function(ref: &mut OffRampState, _: &OwnerCap, function: vector<u8>) {
...
public fun init_execute(
    ...
    assert!(!is_cursed_function(state, b"offramp::init_execute::v1"), EInvalidFunctionVersion);
...
public fun finish_execute(state: &mut OffRampState, receiver_params: osh::ReceiverParams) {
    assert!(!is_cursed_function(state, b"offramp::finish_execute::v1"), EInvalidFunctionVersion);
...
public fun manually_init_execute(
    ...
    assert!(
        !is_cursed_function(state, b"offramp::manually_init_execute::v1"),
...
public fun set_ocr3_config(
    ...
    assert!(!is_cursed_function(state, b"offramp::set_ocr3_config::v1"), EInvalidFunctionVersion);
...
public fun commit(
    ...
    assert!(!is_cursed_function(state, b"offramp::commit::v1"), EInvalidFunctionVersion);
...
public fun set_dynamic_config(
    ...
    assert!(
        !is_cursed_function(state, b"offramp::set_dynamic_config::v1"),
        EInvalidFunctionVersion,
    );
    set_dynamic_config_internal(
        state,
...
public fun apply_source_chain_config_updates(
    ...
    assert!(
        !is_cursed_function(state, b"offramp::apply_source_chain_config_updates::v1"),
        EInvalidFunctionVersion,
    );
    apply_source_chain_config_updates_internal(
        state,
