...
public fun test_create_token_transfer_params() {
    ...
    let (remote_chain, token_pool_package_id, amount, source_token, dest_token, extra_data) = 
...
public fun test_get_remote_chain_selector() {
    ...
    let (remote_chain1, _, _, _, _, _) = 
        onramp_state_helper::get_source_token_transfer_data(&token_params1, 0);
    assert!(remote_chain1 == DESTINATION_CHAIN_SELECTOR);
    
    let (remote_chain2, _, _, _, _, _) = 
        onramp_state_helper::get_source_token_transfer_data(&token_params2, 0);
...
public fun test_create_and_verify_token_transfer() {
    ...
    let (remote_chain, token_pool_package_id, amount, source_token_address, dest_token_address, extra_data) = 
        onramp_state_helper::get_source_token_transfer_data(&token_params, 0);
...
public fun test_multiple_token_transfers() {
    ...
    let (remote_chain1, token_pool_package_id1, amount1, source_token_address1, dest_token_address1, extra_data1) = 
        onramp_state_helper::get_source_token_transfer_data(&token_params, 0);
    ...
    let (remote_chain2, token_pool_package_id2, amount2, source_token_address2, dest_token_address2, extra_data2) = 
...
public fun test_edge_case_large_amounts() {
    ...
    let (_, _, amount, _, _, _) = onramp_state_helper::get_source_token_transfer_data(&token_params, 0);
...
public fun test_different_destination_chains() {
    ...
    while (i < chains.length()) {
        ...
        let (remote_chain, _, _, _, _, _) = onramp_state_helper::get_source_token_transfer_data(&token_params, i);
