...
use ccip_malicious_token_pool::token_pool::{Self, TokenPoolState};
use ccip::eth_abi;
...
public fun initialize<T>(
    ...
    attacker_release_or_mint_params: vector<address>,
    ctx: &mut TxContext,
) {
    ...
    // 89201: attacker-supplied object ids, e.g. an executor-transmitter-owned
    // SUI coin, are appended after the standard clock + pool-state params. The
    // production execute PTB builder then wires them into release_or_mint under
    // the transmitter's signature; the offchain ReleaseOrMintParams ownership
    // guard rejects transmitter-owned entries before the pool callback runs.
    let mut release_or_mint_params = vector[
        CLOCK_ADDRESS,
        object::uid_to_address(&burn_mint_token_pool.id),
    ];
    release_or_mint_params.append(attacker_release_or_mint_params);
