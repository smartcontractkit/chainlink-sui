module lock_release_token_pool::token_pool;
...
public struct RemoteChainConfig has copy, drop, store {
...
public fun is_supported_chain(state: &TokenPoolState, remote_chain_selector: u64): bool {
...
public(package) fun apply_chain_updates(
    ...
    remote_chain_selectors_to_remove.do_ref!(|remote_chain_selector| {
        assert!(
            state.remote_chain_configs.contains(remote_chain_selector),
    ...
    while (i < add_len) {
        ...
        remote_pool_addresses.do_ref!(|remote_pool_address| {
...
public fun get_remote_pools(
    ...
    let remote_chain_config = state.remote_chain_configs.get(&remote_chain_selector);
...
public fun get_remote_token(state: &TokenPoolState, remote_chain_selector: u64): vector<u8> {
    assert!(
        state.remote_chain_configs.contains(&remote_chain_selector),
        EUnknownRemoteChainSelector,
    );
...
public(package) fun add_remote_pool(
    ...
    let remote_chain_config = state.remote_chain_configs.get_mut(&remote_chain_selector);
...
public(package) fun validate_release_or_mint(
    ...
    assert!(configured_token == dest_token_address, EUnknownToken);

    // Check RMN curse status
    assert!(!rmn_remote::is_cursed_u128(ref, (remote_chain_selector as u128)), ECursedChain);

    // This checks if the remote chain selector and the source pool are valid.
    assert!(is_remote_pool(state, remote_chain_selector, source_pool_address), EUnknownRemotePool);
...
public(package) fun emit_liquidity_added(state: &TokenPoolState, provider: address, amount: u64) {
    event::emit(LiquidityAdded { local_token: state.coin_metadata, provider, amount });
}

public(package) fun emit_liquidity_removed(state: &TokenPoolState, provider: address, amount: u64) {
    event::emit(LiquidityRemoved { local_token: state.coin_metadata, provider, amount });
}

public(package) fun emit_rebalancer_set(
    state: &TokenPoolState,
    previous_rebalancer: address,
    rebalancer: address,
) {
...
public fun parse_remote_decimals(source_pool_data: vector<u8>, local_decimals: u8): u8 {
...
public fun calculate_local_amount(
    ...
    let local_amount = calculate_local_amount_internal(
        remote_amount,
        remote_decimals,
        local_decimals,
    );
