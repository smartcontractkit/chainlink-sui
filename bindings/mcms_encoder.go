...
func (e *CCIPEntrypointArgEncoder) EncodeEntryPointArg(executingCallbackParams *transaction.Argument, target, module, function, stateObjID string, data []byte, typeArgs []string) (*bind.EncodedCall, error) {
	clock := codec.Object{Id: "0x6"}
	stateObj := codec.Object{Id: stateObjID}
	registryObj := codec.Object{Id: e.registryObjID}
	deployerStateObj := codec.Object{Id: e.deployerStateObjID}
	...
	encodeWithCCIPObjectRefAndState := func() (*bind.EncodedCall, error) {
		...
		ccipRef := codec.Object{Id: toHexString(ccipRefBytes)}
	...
	case "fee_quoter":
		...
		case "destroy_fee_quoter_cap":
			...
			ccipRef := codec.Object{Id: stateObjID}
			c := codec.Object{Id: toHexString(capBytes)}
	...
	// RECEIVER REGISTRY
	case "receiver_registry":
		receiverRegistry, err := module_receiver_registry.NewReceiverRegistry(target, nil)
		if err != nil {
			return nil, err
		}
		switch function {
		case "unregister_receiver":
			return receiverRegistry.Encoder().McmsUnregisterReceiverWithArgs(stateObj, registryObj, executingCallbackParams)
		default:
			return nil, fmt.Errorf("unsupported receiver_registry MCMS function: %q", function)
		}
	...
	case "offramp":
		...
		case "accept_ownership":
			ccipObjectRef := codec.Object{Id: stateObjID} // For accept_ownership, the state object is the CCIP object ref
			acceptStateObj := codec.Object{Id: toHexString(deserializeFirst32Bytes(data))}

			return offramp.Encoder().McmsAcceptOwnershipWithArgs(ccipObjectRef, acceptStateObj, registryObj, executingCallbackParams)
		default:
	...
	case "onramp":
		...
		case "accept_ownership":
			...
			return onramp.Encoder().McmsAcceptOwnershipWithArgs(ccipObjectRef, acceptStateObj, registryObj, executingCallbackParams)
		...
		case "withdraw_fee_tokens":
			...
			coinMetadata := codec.Object{Id: toHexString(feeTokenMetadata)}
			ccipRef := codec.Object{Id: toHexString(ccipRefBytes)}
	...
	case "lock_release_token_pool":
		...
		case "provide_liquidity":
			deserializer := bcs.NewDeserializer(data)
			deserializer.ReadFixedBytes(SuiAddressLength) // skip state, already known
			deserializer.ReadFixedBytes(SuiAddressLength) // skip rebalancer cap, resolved on-chain
			coinBytes := deserializer.ReadFixedBytes(SuiAddressLength)
			coinObj := codec.Object{Id: toHexString(coinBytes)}

			return lockReleaseTokenPool.Encoder().McmsProvideLiquidityWithArgs(typeArgs, stateObj, registryObj, coinObj, executingCallbackParams)
		case "destroy_token_pool":
			ccipRef := codec.Object{Id: toHexString(deserializeFirst32Bytes(data))}
	...
	case "rmn_remote":
		...
		case "register_curser_cap":
			...
			fastReg := codec.Object{Id: toHexString(fastRegBytes)}
			curserCap := codec.Object{Id: toHexString(curserCapBytes)}
	...
	case "managed_token":
		...
		case "increment_mint_allowance",
			...
			denyListObj := codec.Object{Id: toHexString(denyList)}
	...
	return nil, fmt.Errorf("unsupported MCMS entrypoint callback: module=%q function=%q", module, function)
}
