...
type IOwnable interface {
	New() bind.IMethod
	OwnerCapId(state bind.Object) bind.IMethod
...
func (c *OwnableContract) OwnerCapId(state bind.Object) bind.IMethod {
	build := func(ctx context.Context) (*suiptb.ProgrammableTransactionBuilder, error) {
		// TODO: Object creation is always set to false. Contract analyzer should check if the function uses ::transfer
		ptb, err := bind.BuildPTBFromArgs(ctx, c.client, c.packageID, "ownable", "owner_cap_id", false, "", "", state)
		if err != nil {
			return nil, fmt.Errorf("failed to build PTB for moudule %v in function %v: %w", "ownable", "owner_cap_id", err)
		}
