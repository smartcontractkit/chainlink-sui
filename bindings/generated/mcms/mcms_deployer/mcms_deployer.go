...
const FunctionInfo = `[{"package":"mcms","module":"mcms_deployer","name":"authorize_upgrade","parameters":[{"name":"_","type":"OwnerCap"},{"name":"state","type":"DeployerState"},{"name":"policy","type":"u8"},{"name":"digest","type":"vector<u8>"},{"name":"package_address","type":"address"}]},{"package":"mcms","module":"mcms_deployer","name":"commit_upgrade","parameters":[{"name":"state","type":"DeployerState"},{"name":"receipt","type":"UpgradeReceipt"}]},{"package":"mcms","module":"mcms_deployer","name":"has_upgrade_cap","parameters":[{"name":"state","type":"DeployerState"},{"name":"package_address","type":"address"}]},{"package":"mcms","module":"mcms_deployer","name":"register_upgrade_cap","parameters":[{"name":"state","type":"DeployerState"},{"name":"registry","type":"Registry"},{"name":"upgrade_cap","type":"UpgradeCap"}]},{"package":"mcms","module":"mcms_deployer","name":"release_upgrade_cap","parameters":[{"name":"state","type":"DeployerState"},{"name":"registry","type":"Registry"},{"name":"proof","type":"T"}]},{"package":"mcms","module":"mcms_deployer","name":"release_upgrade_cap_at","parameters":[{"name":"state","type":"DeployerState"},{"name":"registry","type":"Registry"},{"name":"package_address","type":"address"},{"name":"_proof","type":"T"}]}]`
...
type IMcmsDeployer interface {
	...
	ReleaseUpgradeCapAt(ctx context.Context, opts *bind.CallOpts, typeArgs []string, state bind.Object, registry bind.Object, packageAddress string, proof bind.Object) (*models.SuiTransactionBlockResponse, error)
...
type IMcmsDeployerDevInspect interface {
	...
	ReleaseUpgradeCapAt(ctx context.Context, opts *bind.CallOpts, typeArgs []string, state bind.Object, registry bind.Object, packageAddress string, proof bind.Object) (bind.Object, error)
...
type McmsDeployerEncoder interface {
	...
	ReleaseUpgradeCapAt(typeArgs []string, state bind.Object, registry bind.Object, packageAddress string, proof bind.Object) (*bind.EncodedCall, error)
	ReleaseUpgradeCapAtWithArgs(typeArgs []string, args ...any) (*bind.EncodedCall, error)
...
// ReleaseUpgradeCapAt executes the release_upgrade_cap_at Move function.
func (c *McmsDeployerContract) ReleaseUpgradeCapAt(ctx context.Context, opts *bind.CallOpts, typeArgs []string, state bind.Object, registry bind.Object, packageAddress string, proof bind.Object) (*models.SuiTransactionBlockResponse, error) {
	encoded, err := c.mcmsDeployerEncoder.ReleaseUpgradeCapAt(typeArgs, state, registry, packageAddress, proof)
	if err != nil {
		return nil, fmt.Errorf("failed to encode function call: %w", err)
	}
...
// ReleaseUpgradeCapAt executes the release_upgrade_cap_at Move function using DevInspect to get return values.
//
// Returns: UpgradeCap
func (d *McmsDeployerDevInspect) ReleaseUpgradeCapAt(ctx context.Context, opts *bind.CallOpts, typeArgs []string, state bind.Object, registry bind.Object, packageAddress string, proof bind.Object) (bind.Object, error) {
	encoded, err := d.contract.mcmsDeployerEncoder.ReleaseUpgradeCapAt(typeArgs, state, registry, packageAddress, proof)
	if err != nil {
		return bind.Object{}, fmt.Errorf("failed to encode function call: %w", err)
	}
	results, err := d.contract.Call(ctx, opts, encoded)
...
// ReleaseUpgradeCapAt encodes a call to the release_upgrade_cap_at Move function.
func (c mcmsDeployerEncoder) ReleaseUpgradeCapAt(typeArgs []string, state bind.Object, registry bind.Object, packageAddress string, proof bind.Object) (*bind.EncodedCall, error) {
	typeArgsList := typeArgs
	typeParamsList := []string{
		"T",
	}
	return c.EncodeCallArgsWithGenerics("release_upgrade_cap_at", typeArgsList, typeParamsList, []string{
		"&mut DeployerState",
		"&Registry",
		"address",
		"T",
...
// ReleaseUpgradeCapAtWithArgs encodes a call to the release_upgrade_cap_at Move function using arbitrary arguments.
// This method allows passing both regular values and transaction.Argument values for PTB chaining.
func (c mcmsDeployerEncoder) ReleaseUpgradeCapAtWithArgs(typeArgs []string, args ...any) (*bind.EncodedCall, error) {
	expectedParams := []string{
		"&mut DeployerState",
		"&Registry",
		"address",
	...
	return c.EncodeCallArgsWithGenerics("release_upgrade_cap_at", typeArgsList, typeParamsList, expectedParams, args, []string{
		"UpgradeCap",
	})
}
