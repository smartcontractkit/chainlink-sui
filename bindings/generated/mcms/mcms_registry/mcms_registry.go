...
type McmsRegistryEncoder interface {
	...
	GetCallbackParams(typeArgs []string, registry bind.Object, params ExecutingCallbackParams, proof bind.Object) (*bind.EncodedCall, error)
...
type bcsModulesAdded struct {
	RegistryId     bind.Object
	PackageAddress [32]byte
	ModuleNames    [][]byte
}

func convertModulesAddedFromBCS(bcs bcsModulesAdded) (ModulesAdded, error) {

	return ModulesAdded{
		RegistryId:     bcs.RegistryId,
		PackageAddress: fmt.Sprintf("0x%x", bcs.PackageAddress),
		ModuleNames:    bcs.ModuleNames,
	}, nil
}

type bcsModulesRemoved struct {
	RegistryId     bind.Object
	PackageAddress [32]byte
	ModuleNames    [][]byte
}

func convertModulesRemovedFromBCS(bcs bcsModulesRemoved) (ModulesRemoved, error) {
...
func init() {
	...
	bind.RegisterStructDecoder("mcms::mcms_registry::ModulesAdded", func(data []byte) (interface{}, error) {
		var temp bcsModulesAdded
		_, err := mystenbcs.Unmarshal(data, &temp)
		if err != nil {
			return nil, err
		}

		result, err := convertModulesAddedFromBCS(temp)
		if err != nil {
			return nil, err
		}
		return result, nil
	...
	bind.RegisterStructDecoder("vector<mcms::mcms_registry::ModulesAdded>", func(data []byte) (interface{}, error) {
		var temps []bcsModulesAdded
		_, err := mystenbcs.Unmarshal(data, &temps)
		if err != nil {
			return nil, err
		}

		results := make([]ModulesAdded, len(temps))
		for i, temp := range temps {
			result, err := convertModulesAddedFromBCS(temp)
			if err != nil {
				return nil, fmt.Errorf("failed to convert element %d: %w", i, err)
	...
	bind.RegisterStructDecoder("mcms::mcms_registry::ModulesRemoved", func(data []byte) (interface{}, error) {
		var temp bcsModulesRemoved
		_, err := mystenbcs.Unmarshal(data, &temp)
		if err != nil {
			return nil, err
		}

		result, err := convertModulesRemovedFromBCS(temp)
		if err != nil {
			return nil, err
		}
		return result, nil
	...
	bind.RegisterStructDecoder("vector<mcms::mcms_registry::ModulesRemoved>", func(data []byte) (interface{}, error) {
		var temps []bcsModulesRemoved
		_, err := mystenbcs.Unmarshal(data, &temps)
		if err != nil {
			return nil, err
		}

		results := make([]ModulesRemoved, len(temps))
		for i, temp := range temps {
			result, err := convertModulesRemovedFromBCS(temp)
			if err != nil {
				return nil, fmt.Errorf("failed to convert element %d: %w", i, err)
...
func (c *McmsRegistryContract) GetCallbackParams(ctx context.Context, opts *bind.CallOpts, typeArgs []string, registry bind.Object, params ExecutingCallbackParams, proof bind.Object) (*models.SuiTransactionBlockResponse, error) {
	encoded, err := c.mcmsRegistryEncoder.GetCallbackParams(typeArgs, registry, params, proof)
	if err != nil {
		return nil, fmt.Errorf("failed to encode function call: %w", err)
	}
...
//	[2]: 0x1::string::String
//	[3]: vector<u8>
func (d *McmsRegistryDevInspect) GetCallbackParams(ctx context.Context, opts *bind.CallOpts, typeArgs []string, registry bind.Object, params ExecutingCallbackParams, proof bind.Object) ([]any, error) {
	encoded, err := d.contract.mcmsRegistryEncoder.GetCallbackParams(typeArgs, registry, params, proof)
	if err != nil {
		return nil, fmt.Errorf("failed to encode function call: %w", err)
	}
	return d.contract.Call(ctx, opts, encoded)
...
func (c mcmsRegistryEncoder) GetCallbackParams(typeArgs []string, registry bind.Object, params ExecutingCallbackParams, proof bind.Object) (*bind.EncodedCall, error) {
	typeArgsList := typeArgs
	typeParamsList := []string{
		"T",
	}
