...
func TestCounterAutoGenerated(t *testing.T) {
	...
	t.Run("TwoIncrements", func(t *testing.T) {
		// Increment twice.
		increment := counter.Increment(counterObject)
		...
		_, err = counter.GetCount(counterObject).Inspect(ctx, bind.TxOpts{}, signer, *client)
		require.NoError(t, err)
	...
	t.Run("IncrementMult", func(t *testing.T) {
		...
		incrementMult := counter.IncrementMult(counterObject, 4, 5)
