...
func compilePackageInternal(packageName contracts.Package, namedAddresses map[string]string, isUpgrade bool, suiRPC string, modifier SourceModifier) (PackageArtifact, error) {
	...
	if isUpgrade {
		...
		cmd = exec.Command("sui", "client", "test-publish", "--build-env", "testnet", "--environment", env, "--with-unpublished-dependencies", "--serialize-unsigned-transaction", "--sender", namedAddresses["signer"], "--json")
		cmd.Dir = packageRoot
		output, err := cmd.Output()
		if err != nil {
