...
func (tIndexer *TransactionsIndexer) processFailedTransaction(
	...
	for i, cmd := range programmableTx.Commands {
		...
		if (pkg == eventAccountAddress || pkg == latestOfframpPackageID) &&
			...
			isExecuteInitFunction(function) {
			executionMethodIndex = i
			detectedExecuteFunction = function
	...
	if !includesValidPTBCommand {
		tIndexer.logger.Warnw(
			"Expected PTB command (_::offramp::init_execute or init_execute_v2) not found",
			"txDigest", tx.GetDigest(),
		)
		return nil, nil
	}

	// The failure should NOT take place at init_execute/init_execute_v2.
	if moveAbort.Location.FunctionName != nil &&
		*moveAbort.Location.FunctionName == detectedExecuteFunction {
		tIndexer.logger.Debugw("Skipping - failure at init execute function",
			"txDigest", tx.GetDigest(),
			"functionName", detectedExecuteFunction)
		return nil, nil
	}
	...
	if detectedExecuteFunction == executeFunctionV2 {
		reportBytesUnwrapped, err := codec.UnwrapBCSPureBytes(reportBytes)
		if err != nil {
			return nil, fmt.Errorf("failed to unwrap v2 execution report pure bytes: %w", err)
		}

		execReportV2, err := codec.DeserializeExecutionReportV2(reportBytesUnwrapped)
		if err != nil {
			return nil, fmt.Errorf("failed to deserialize execution report v2: %w", err)
		}
		...
		hasher := chainreader_util.NewMessageHasherV2(tIndexer.logger)
		messageHash, err = hasher.HashExecutionReportV2(ctx, execReportV2, sourceChainConfig.OnRamp)
		if err != nil {
			return nil, fmt.Errorf("failed to calculate message hash v2: %w", err)
		}
