...
#### 1. DBStore
The main database interface providing CRUD operations. It wraps a `sqlutil.DataSource` (the connection/pool is owned by Chainlink Core and shared across relayer components):
...
func NewDBStore(ds sqlutil.DataSource, lgr logger.Logger) *DBStore {
    return &DBStore{ds: ds, lgr: lgr}
}
```

> All tables live under a dedicated `sui` schema (e.g. `sui.events`). `EnsureSchema` creates the schema, tables, and indexes if they do not already exist; it is idempotent and called on startup.
...
The primary table for storing blockchain events (both real and synthetic). This is the actual schema created by `EnsureSchema`:
...
The `UNIQUE` constraint plus `ON CONFLICT DO NOTHING` on insert makes re-indexing a checkpoint idempotent (important for the ChainPoller's rescans).
...
| `event_offset` | BIGINT | Stable per-handle offset for ordering (`0` for synthetic events) |
| `tx_digest` | TEXT | Transaction digest (hex) containing the event |
| `block_version` | BIGINT | Reserved (currently `0`) |
| `block_height` | TEXT | Checkpoint sequence number (as string) |
| `block_hash` | BYTEA | Checkpoint digest bytes |
| `block_timestamp` | BIGINT | Checkpoint timestamp (seconds) |
| `data` | JSONB | Event payload as structured JSON (camelCase keys) |
| `is_synthetic` | BOOLEAN | `true` for synthetic failure events generated by the Transactions Indexer |
...
### Transmitter Cursors Table

The indexing pipeline also maintains a small table for transmitter cursor bookkeeping:
...
> **Note**: The Transaction Manager persists its own transaction state separately (outside the ChainReader's `sui` schema). See the [Transaction Manager documentation](./transaction-manager.md) for its store.
...
`InsertEvents` writes each record with an idempotent insert (`ON CONFLICT DO NOTHING`). The `EventRecord` carries the `IsSynthetic` flag so real and synthetic events share one code path:
...
INSERT INTO sui.events (
    event_account_address, event_handle, event_offset, tx_digest,
    block_version, block_height, block_hash, block_timestamp, data, is_synthetic
...
func (store *DBStore) InsertEvents(ctx context.Context, records []EventRecord) error {
    for _, record := range records {
        ...
        _, err = store.ds.ExecContext(ctx, InsertEvent,
            record.EventAccountAddress, record.EventHandle, record.EventOffset, record.TxDigest,
            record.BlockVersion, record.BlockHeight, record.BlockHash, record.BlockTimestamp,
            data, record.IsSynthetic,
        )
        if err != nil {
...
> The indexers call `InsertEvents` with a slice (a "batch") and fall back to per-record inserts if a batch fails, so one bad record does not drop the rest of a checkpoint's events.
...
`QueryEvents` filters by account address and event handle, applies `chainlink-common` query expressions (which it translates to SQL `WHERE` conditions), and orders by `(block_height::BIGINT, event_offset)`:

```go
func (store *DBStore) QueryEvents(
    ctx context.Context,
    eventAccountAddress, eventHandle string,
    expressions []query.Expression,
    limitAndSort query.LimitAndSort,
) ([]EventRecord, error) {
    baseSQL := QueryEventsBase // SELECT ... WHERE event_account_address = $1 AND event_handle = $2

    args := []any{eventAccountAddress, eventHandle}
    argCount := 3

    // Translate each query.Expression into a parameterized SQL condition.
    for _, expr := range expressions {
        cond, err := BuildSQLCondition(expr, &args, &argCount)
        if err != nil {
    ...
    // Sort by (block_height, event_offset) for deterministic checkpoint ordering.
    // ...apply ORDER BY + LIMIT, execute, scan into []EventRecord...
}
```

The query expressions come from `QueryKey`'s `KeyFilter` and support comparisons against JSONB fields (e.g. `data->>'sourceChainSelector' = $n`), enabling value filtering on event payloads.
...
These are the indexes actually created by `EnsureSchema`:

```sql
-- Account + handle + timestamp for time-ordered queries
CREATE INDEX IF NOT EXISTS idx_events_account_handle_timestamp
    ON sui.events(event_account_address, event_handle, block_timestamp DESC);

-- Offset tracking (per account + handle)
CREATE INDEX IF NOT EXISTS idx_events_offset
    ON sui.events(event_account_address, event_handle, event_offset);

-- GIN index for JSONB payload queries
CREATE INDEX IF NOT EXISTS idx_events_data_gin
    ON sui.events USING gin(data);

-- Account + handle + id for latest-first lookups
CREATE INDEX IF NOT EXISTS idx_events_account_handle_id
    ON sui.events(event_account_address, event_handle, id DESC);
```

> Queries order by `(block_height::BIGINT, event_offset)` to get deterministic checkpoint ordering. The additional JSONB path indexes shown elsewhere in this document are illustrative optimizations, not part of the default schema.
